...
void PlotRoot() {
	...
	// PlotNames.push_back("BlipVertexDistAllNeutron");
	// PlotNames.push_back("BlipProxTrkDistAllNeutron");
	// PlotNames.push_back("BlipVertexDistOverNeutron3");
	// PlotNames.push_back("BlipProxTrkDistOverNeutron3");
	...
	if(!stackedHist){
	  ...
	  for (int iPlot = 0; iPlot < NPlots; iPlot++) {
	    ...
	    for (int iSample = 0; iSample < NSamples; iSample++) {	
	      ...
	      Histos[iSample]->GetYaxis()->SetTitle("Weighted Events / 8.033E20 POT "); //1.62E20 POT ");
	      ...
	      leg->AddEntry(Histos[iSample],Labels[iSample] + "  (" + to_string(Histos[iSample]->Integral() / all_integral) +  ")  " ,"l"); // + to_string(Histos[iSample]->Integral())  ,"l");
